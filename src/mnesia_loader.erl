...
ext_init_table(Action, Alias, Mod, Tab, Fun, State, Sender) ->
	...
	% VRV. Dump the queue of writes to the table.
	% TODO. Can we safely apply changes during table initialization?
	% Probably we need to dump them to a file and apply them only on finish
	CS = val({Tab, cstruct}),
	TabRef = {{ext,Alias,Mod}, Tab},
