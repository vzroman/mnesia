...
init(Parent) ->
	...
	% VRV. init a pool of processes for async dirty operations
	spawn_link(fun init_async_pool/0),

    doit_loop(#state{supervisor = Parent}).

% VRV. Async pool implementation
init_async_pool()->
	% Register the accepting process
	Self = self(),
	register(mnesia_tm_async, Self),
	% Enter the loop
	async_accept(#state{}).
...
% 	try
% 		Top=
% 			lists:sublist(lists:reverse(lists:sort([{erlang:process_info(PID,[memory]),PID}||PID<-erlang:processes()])),5),
% 		Info=	
% 			[begin 
% 				#{
% 					memory:=Memory,
...
% 				} = maps:from_list(erlang:process_info(PID,[memory,message_queue_len,current_stacktrace])),
% 				Messages=
% 					if
% 						QueueLength>100->
% 							lists:sublist(proplists:get_value(messages,erlang:process_info(PID,[messages])),5);
% 						true->[]
% 					end,	
% 				[{memory,Memory},{message_queue_len,QueueLength},{messages,Messages},{stack,Stack}]
% 			end || {_,PID} <- Top ],

% 		fp:log(info,"tmp_monitor: ~p",[Info])	
% 	catch
% 		_:Error->
% 			fp:log(error,"tmp_monitor error: ~p",[Error])
% 	end,
% 	tmp_monitor().
		...
		receive
			...
			{Tab, unblock_me, From}->
				case lists:member(Tab, State#state.blocked_tabs) of
					false -> State;
					true ->
						State#state{dirty_queue = [ {Tab, unblock_me, From} | State#state.dirty_queue ] }
				end;
			{block_tab, Tab}->
				State#state{blocked_tabs = [Tab | State#state.blocked_tabs]};
			{unblock_tab, Tab}->
				...
				case lists:member(Tab, BlockedTabs2) of
					false ->
						...
						State#state{blocked_tabs = BlockedTabs2, dirty_queue = Queue};
					true ->
						State#state{blocked_tabs = BlockedTabs2}
				end;
			Msg when not (is_tuple(Msg) andalso element(1,Msg) =:= async_worker_ready )->
				mnesia_lib:warning("async pool unexpected message ~p~n",[Msg])
		end,
	async_accept( State1 ).

% Async pool API
async_pool_request( Req )->
	mnesia_tm_async ! Req.
...
doit_loop(#state{coordinators=Coordinators,participants=Participants,supervisor=Sup}=State) ->
	receive
		% VRV. Move dirty operations to the async_pool
	{_From, {async_dirty, Tid, Commit, Tab}} ->
	    async_pool_request({_From, {async_dirty, Tid, Commit, Tab}}),
		doit_loop(State);
	{From, {sync_dirty, Tid, Commit, Tab}} ->
	    async_pool_request({From, {sync_dirty, Tid, Commit, Tab}}),
		doit_loop(State);
	...
	{From, {unblock_me, Tab}} ->
		% VRV. Update async pool manager
		Item = {Tab, unblock_me, From},
		async_pool_request( Item ),
	...
	{From, {unblock_tab, Tab}} ->
		% VRV. Update async pool manager
		async_pool_request( {unblock_tab, Tab} ),
...
sync_send_dirty(Tid, [Head | Tail], Tab, WaitFor) ->
	...
	Node == node() ->
	    ...
	    {WF, Res};
	true ->
		% VRV. Dirty operations optimization
		{mnesia_tm_async, Node} ! {self(), {sync_dirty, Tid, Head, Tab}},
...
async_send_dirty(Tid, [Head | Tail], Tab, ReadNode, WaitFor, Res) ->
	...
	true ->
		% VRV dirty operations optimization
		{mnesia_tm_async, Node} ! {self(), {async_dirty, Tid, Head, Tab}},
